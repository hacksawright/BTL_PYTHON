...
# ======================================================
# ⚽ Danh sách 20 đội Premier League mùa 2024–2025
# ======================================================
teams = {
    "Arsenal": "18bb7c10",
...
def scrape_team(team_name, team_id):
    ...
    df = df.loc[:, ~df.columns.duplicated()]  # loại cột trùng tên

    # loại cột trùng nội dung
    to_drop = []
    ...
    df.to_csv(f"{team_name.replace(' ', '_')}_full_{SEASON}_clean.csv", index=False, encoding="utf-8-sig")
...
for team_name, team_id in teams.items():
    try:
        ...
        print(f"⚠️ Lỗi {team_name}: {e}")
        continue

# ======================================================
# 🧩 Gộp toàn bộ đội và lưu vào SQLite
# ======================================================
if all_teams_data:
    combined = pd.concat(all_teams_data, ignore_index=True).fillna("N/a")
    combined.to_csv("PremierLeague_2024-2025_AllTeams.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"\n🏆 Hoàn tất toàn bộ 20 đội Premier League {SEASON}")
    print(f"📈 Tổng cộng: {len(combined)} cầu thủ, {len(combined.columns)} cột")
    print("💾 Đã lưu vào premierleague_2024_25.db (bảng: player_stats)")
else:
